...
The summarizedData.csv table has exactly the same structure as the first table, but data was summarized so that only one row exist for each combination of activity and subject (feature columns are averaged).
...
Note that in to generate the summary, the reshape2 R package was used, which needs to be installed in order for the scripts to work.