...
{
  # Read the data
  rawData <- read.fwf(sprintf("UCI HAR Dataset/%s/X_%s.txt", set, set),widths=rep(16,561))
  ...
  activityTable <- read.table(sprintf("UCI HAR Dataset/%s/y_%s.txt", set, set),colClasses = "factor")
  ...
  subject <- read.table(sprintf("UCI HAR Dataset/%s/subject_%s.txt", set, set),colClasses = "factor")
